...
class TXSimulationBackend:
    ...
    def __init__(self, params: VirtualChannelParams | None = None) -> None:
        ...
        if ok:
            ...
            self._log_append({"time": time.strftime("%H:%M:%S"), "event": "Error", "details": f"UDP bind failed on port 9902: {err}"})
    ...
    def stop(self) -> None:
        """Stop timers and close sockets gracefully."""
    ...
    def _send_heartbeat(self) -> None:
        import json
        ping_msg = {"event": "ping"}
        self._socket.writeDatagram(json.dumps(ping_msg).encode("utf-8"), QHostAddress(QHostAddress.SpecialAddress.LocalHost), 9901)
    ...
    def _on_ready_read(self) -> None:
        ...
        while self._socket.hasPendingDatagrams():
            ...
            try:
                msg = json.loads(data.data().decode("utf-8", errors="replace"))
    ...
    def start_transmission(self, filepath: str) -> None:
        ...
        path = Path(filepath)
        if not path.exists():
            self._log_append({"time": time.strftime("%H:%M:%S"), "event": "Error", "details": f"File {path.name} not found"})
            return
        ...
        self._log.clear()
        self._log_append({"time": time.strftime("%H:%M:%S"), "event": "Start TX", "details": f"Sending {self._filename} ({self._total_chunks} chunks)"})
        self._transfer_id = int(time.time() * 10) % 100000
        start_msg = {"event": "start_tx", "filename": self._filename, "total_chunks": self._total_chunks, "chunk_size": self._chunk_size, "tid": self._transfer_id}
        self._socket.writeDatagram(json.dumps(start_msg).encode("utf-8"), QHostAddress(QHostAddress.SpecialAddress.LocalHost), 9901)
    ...
    def _send_next_chunk(self) -> None:
        ...
        chunk_msg = {"event": "chunk", "index": self._current_chunk, "data": chunk_bytes.hex()}
        self._socket.writeDatagram(json.dumps(chunk_msg).encode("utf-8"), QHostAddress(QHostAddress.SpecialAddress.LocalHost), 9901)
        ...
        if self._current_chunk >= self._total_chunks:
            ...
            self._socket.writeDatagram(json.dumps({"event": "stop_tx"}).encode("utf-8"), QHostAddress(QHostAddress.SpecialAddress.LocalHost), 9901)
    ...
    def _compute_channel(self) -> None:
        ...
        p.snr_db = 20 * math.log10(max(signal / ambient_noise, 1.0)) if ambient_noise > 0 else 60.0

    def _build_log(self) -> list[dict[str, str]]:
        if self._log:
            return self._log[-10:]
        now = time.strftime("%H:%M:%S")
        log: list[dict[str, str]] = [{"time": now, "event": "System ready"}]
    ...
    def refresh(self) -> TXSimulationSnapshot:
        ...
        return TXSimulationSnapshot(filename=filename, filetype="Binary Data" if self._filename != "No file" else "", file_size_bytes=file_size, total_chunks=total_chunks, chunk_size=p.chunk_size, encoding=p.encoding, modulation="NRZ / OOK", symbol_rate=f"{p.symbol_hz:,} sym/s", led_pin=5, tx_power="100 %", pre_emphasis="Enabled", status_text=self._status_text, progress_percent=int(self._progress), current_chunk=current, elapsed_time=elapsed_str, estimated_time=est_remaining, data_rate=data_rate_str, port="—", serial_connected=self._rx_connected, activity_log=self._build_log(), record_count=1, tid=self._transfer_id, channel_params=p)
