...
class LogsPage(QWidget):
    def __init__(self, state: TXAppState | None = None, parent: QWidget | None = None) -> None:
        ...
        lhdr.setSectionResizeMode(3, QHeaderView.ResizeMode.ResizeToContents) # Chunks Sent
        ...
        self.logs_table.setColumnWidth(0, 90)   # Timestamp (HH:MM:SS)
        self.logs_table.setColumnWidth(2, 150)  # Outcome  (SUCCESS / FAILED + badges)
        self.logs_table.setColumnWidth(4, 120)  # Data Rate (e.g. "12345 bps")
        self.logs_table.setColumnWidth(5, 100)  # Duration (HH:MM:SS)
    ...
    def refresh(self, state: TXAppState) -> None:
        ...
        for row, entry in enumerate(state.session_history):
            ...
            # Collect numeric rate in bps for average (handles "12345 bps" or "12.3 kbps")
            try:
        ...
        if rates:
            avg_bps = sum(rates) / len(rates)
            if avg_bps >= 1000:
                avg_txt = f"Avg Rate: {avg_bps / 1000:.1f} kbps"
            else:
                avg_txt = f"Avg Rate: {avg_bps:.0f} bps"
            self._stat_avg_rate.setText(avg_txt)
        else:
