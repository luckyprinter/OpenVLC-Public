...
class TransmissionPipelineWidget(Card):
    ...
    def refresh(self, state: TXAppState) -> None:
        ...
        is_transmitting = "transmitting" in status_lower and "complete" not in status_lower and "failed" not in status_lower
...
class CurrentSessionWidget(Card):
    ...
    def refresh(self, state: TXAppState) -> None:
        ...
        # Compute average throughput from completed transmissions
        rates_bps: list[float] = []
        for item in state.session_history:
            if item.get("outcome") == "SUCCESS":
                rate_str = item.get("throughput", "")
                try:
                    ...
                    if len(parts) > 1 and "kbps" in parts[1].lower():
                        val *= 1000.0
                    if val > 0:
                        rates_bps.append(val)
                except (ValueError, IndexError):
                    pass
        
        if rates_bps:
            avg = sum(rates_bps) / len(rates_bps)
            avg_str = f"{avg / 1000:.1f} kbps" if avg >= 1000 else f"{avg:.0f} bps"
        else:
            avg_str = "—"
        self.row_rate.setValue(avg_str)
...
class FileDetailsWidget(Card):
    ...
    def refresh(self, state: TXAppState) -> None:
        ...
        if "complete" in status_lower:
            self.row_time.lbl.setText("Transmission Time")
            self.row_time.setValue(state.elapsed_time if state.elapsed_time not in ("00:00:00", "", None) else "—")
        else:
            self.row_time.lbl.setText("Est. Duration")
            # estimated_time may be "300 ms" or "00:00:05" — show it if non-empty
            est = state.estimated_time or ""
            self.row_time.setValue(est if est and est != "00:00:00" else "—")
        ...
        elif "failed" in status_lower or "send failed" in status_lower:
            self.row_crc.setValue("FAILED (CRC Error)", COLORS["red"])
        elif status_lower.startswith("selected") or "idle" in status_lower or not status_lower:
...
class DigitalBitstreamWidget(Card):
    ...
    def _paint_canvas(self, event: Any) -> None:
        ...
        painter.setPen(QPen(wave_color, 2, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap, Qt.PenJoinStyle.RoundJoin))
...
class _TXLogPanel(Card):
    ...
    def refresh(self, state: TXAppState) -> None:
        ...
        if log_entries:
            if getattr(self, "_last_log_len", 0) == len(log_entries):
                return
        ...
        # Build new - limit to top 100 for UI performance
        for i, entry in enumerate(reversed(log_entries[-100:])):
...
class TXDashboardPage(QWidget):
    ...
    def _on_select_file_clicked(self) -> None:
        ...
        if filepath:
            ...
            if p.exists():
                ...
                # 4B5B: 10 bits per byte of data
                est_seconds = (self._state.file_size_bytes * 10) / max(1, symbol_hz)
                
                if est_seconds < 1.0:
                    self._state.estimated_time = f"{est_seconds * 1000:.0f} ms"
                else:
                    m, s = divmod(int(est_seconds), 60)
                    h, m = divmod(m, 60)
                    self._state.estimated_time = f"{h:02d}:{m:02d}:{s:02d}"
                
                self._state.status_text = f"Selected {p.name}"
                self._pending_filepath = filepath
                file_log("TX", "User Action", f"Selected file: {p.name} ({self._state.file_size_bytes} bytes)")
                
                # Update UI directly without wiping selection state
                self.refresh()
    ...
    def _on_start_clicked(self) -> None:
        ...
        if filepath:
            file_log("TX", "User Action", f"Clicked Start Transmission for {Path(filepath).name}")
