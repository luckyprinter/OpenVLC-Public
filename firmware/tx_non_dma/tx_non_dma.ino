...
void send4b5bPreambleAndSync() {
  ...
  for (int i = 0; i < PREAMBLE_BITS; i++) send4b5bBit(((i & 1) == 0) ? idleOn : !idleOn);
...
void setup() {
  ...
  analogWriteFrequency(TX_PIN, PWM_CARRIER_HZ);
  ...
  Serial.println("Commands: MODE=4B5B, FREQ, GAP, FGAP, ACTIVE_LOW, IDLE_ON, QUIET, STREAM_*");
  reportLineConfig();
  Serial.println();
}
